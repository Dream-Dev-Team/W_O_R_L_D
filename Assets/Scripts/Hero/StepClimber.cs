...
public class StepClimber : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (stepUp)
        {
            this.GetComponent<Rigidbody2D>().position += stepUpOffset;
    ...
    bool FindStep(out Vector2 stepUpOffset, List<ContactPoint> allCPs, ContactPoint groundCP, Vector2 currVelocity)
    {
        stepUpOffset = default(Vector3);

        //No chance to step if the player is not moving
        Vector2 velocityX = new Vector2(currVelocity.x, currVelocity.y);
        if (velocityX.sqrMagnitude < 0.0001f)
            return false;
    ...
    bool ResolveStepUp(out Vector2 stepUpOffset, ContactPoint stepTestCP, ContactPoint groundCP)
    {
